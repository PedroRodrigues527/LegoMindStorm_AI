...
void inserirInimigo(int pos, string str, int forca)
{
	...
	for(int j = 1; j < 3; j++)
	{
		...
		{
		case A_MAX_FORCA: //artilharia
			arrayenemies[pos][j] = artilharia[j]; //{{500,1,50},...}
			break;
		case T_MAX_FORCA: //tanque
			arrayenemies[pos][j] = tanque[j];
			break;
		case i_MAX_FORCA: //infantaria
...
void turnoDB()
{
	...
	if(db[1] + intrec > DB_MAX_EN) //se energia antes + energia de recuperação em int exceder o máximo de energia possível
	{
		intrec = DB_MAX_EN - db[1]; //energia recuperação vai ter o valor restante para 500 de energia
		db[1] = DB_MAX_EN; //energia de DB vai ser o máximo - 500
	}
	else
	{
	...
	for(int i = 0; i < 6; i++)
	{
		if(arrayenemies[i][0] == A_MAX_FORCA)
		{
			...
			else if(db[1] >= DB_MAX_VIDA*0.2 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[0][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
			...
			else if(db[1] >= DB_MAX_VIDA*0.2 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[1][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
			...
			if(db[1] >= DB_MAX_VIDA*0.2 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[2][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
	...
	for(int i = 0; i < 6; i++)
	{
		if(arrayenemies[i][0] == T_MAX_FORCA)
		{
			//é tanque
			//se energia de db >= 150, num_ataque > 0, artilharia está a 50 unidades de vida e ainda não foi atacado por DB
			if(db[1]>= DB_MAX_VIDA*0.6 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[0][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
			...
			else if(db[1]>= DB_MAX_VIDA*0.45 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[1][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
			...
			else if(db[1] >= DB_MAX_VIDA*0.3 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[2][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
	...
	for(int i = 0; i < 6; i++)
	{
		if(arrayenemies[i][0] == I_MAX_FORCA)
		{
			//é infantaria
			if(db[1]>= DB_MAX_VIDA*0.6 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[0][0])
			{
			...
			else if(db[1]>= DB_MAX_VIDA*0.4 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[1][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
			...
			else if(db[1] >= DB_MAX_VIDA*0.25 && arrayenemies[i][1] > 0 && arrayenemies[i][2] >= db_ataques[2][0])
			{
				//MOTOR - RODAS PARA FRENTE
				movimento(i);
				sleep(1000); //1 segundo
